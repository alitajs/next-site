...
Create a [`pages/_app.js` file](https://nextjs-cn.com/docs/advanced-features/custom-app) if not already present.
...
Since stylesheets are global by nature, they must be imported in the [Custom `<App>` component](https://nextjs-cn.com/docs/advanced-features/custom-app).
...
Regular [`<link>` stylesheets](https://nextjs-cn.com/docs/api-reference/next/head) and [global CSS files](#built-in-css-support-for-global-stylesheets) are still supported.
...
With the release of Next.js 9 we introduced [dynamic route segments](https://nextjs-cn.com/blog/next-9#dynamic-route-segments) with the goal of simplifying dynamic segments in Next.js without the need for a custom server.
