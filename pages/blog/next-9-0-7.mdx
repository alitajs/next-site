...
Next.js also handles type-checking by running `tsc --watch` in parallel to the development process. During development, Next.js has a concept called on-demand entries. This feature only compiles pages that you are actively working on — you can read more about it [here](https://nextjs-cn.com/blog/next-8#improved-on-demand-entries).
...
&#8203;<OrgText /> has a few large Next.js applications. For example, <PlatformLink href='/' />, <PlatformLink href='/docs' />, and [https://nextjs-cn.com](https://nextjs-cn.com). We've been [dogfooding](https://en.wikipedia.org/wiki/Eating_your_own_dog_food) Next.js internally at <OrgText /> and improved Next.js based on our experiences.
...
You can read more about telemetry on [nextjs-cn.com/telemetry](https://nextjs-cn.com/telemetry). There you will also find how to optionally opt-out if you don’t want to participate.
...
In doing so, the `next-server` became obsolete and was replaced by the [serverless target](https://nextjs-cn.com/docs#serverless-deployment). This target has a far more optimized output than using the `next-server` package as a replacement for `next`.
