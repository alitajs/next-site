...
Next.js 9.0 introduced the concept of [Automatic Static Optimization](https://nextjs-cn.com/blog/next-9#automatic-static-optimization). When a page does not have blocking data fetching requirements like `getInitialProps`, it will be automatically rendered to HTML at build time.
...
Today we're incredibly excited to announce two new data fetching methods: [`getStaticProps`](https://nextjs-cn.com/docs/basic-features/data-fetching) and [`getServerSideProps`](https://nextjs-cn.com/docs/basic-features/data-fetching). We also include a way to provide parameters to statically generate static pages for dynamic routes: [`getStaticPaths`](https://nextjs-cn.com/docs/basic-features/data-fetching).
...
- `getStaticPaths` (Static Generation): Specify [dynamic routes](https://nextjs-cn.com/docs/routing/dynamic-routes) to prerender based on data.
...
- `params`: `params` contains the route parameters for pages using dynamic routes. For example, if the page name is `[id].js` , then `params` will look like `{ id: ... }`. To learn more, take a look at the [Dynamic Routing documentation](https://nextjs-cn.com/docs/routing/dynamic-routes). You should use this together with `getStaticPaths`, which we’ll explain later.
...
To learn more about `getStaticProps` refer to the [Data Fetching Documentation](https://nextjs-cn.com/docs/basic-features/data-fetching).

### getStaticPaths

If a page has [dynamic routes](https://nextjs-cn.com/docs/routing/dynamic-routes) and uses `getStaticProps` it needs to define a list of paths that have to be rendered to HTML at build time.
...
To learn more about `getStaticPaths` refer to the [Data Fetching Documentation](https://nextjs-cn.com/docs/basic-features/data-fetching).
...
- `params`: If this page uses a dynamic route, `params` contains the route parameters. If the page name is `[id].js` , then `params` will look like `{ id: ... }`. To learn more, take a look at the [Dynamic Routing documentation](https://nextjs-cn.com/docs/routing/dynamic-routes).
...
To learn more about `getServerSideProps` refer to the [Data Fetching Documentation](https://nextjs-cn.com/docs/basic-features/data-fetching).
...
Preview Mode is already available when using `next start`, or seamlessly by [deploying](https://nextjs-cn.com/docs/deployment) to the <PlatformCdnLink />.
...
Learn more about [Preview Mode](https://nextjs-cn.com/docs/advanced-features/preview-mode) by referencing [the documentation](https://nextjs-cn.com/docs/advanced-features/preview-mode).
...
[Next.js 9.2](https://nextjs-cn.com/blog/next-9-2) introduced [built-in support for Global CSS Stylesheets](https://nextjs-cn.com/blog/next-9-2#built-in-css-support-for-global-stylesheets) to replace the `next-css` plugin with better defaults to provide a more optimized result.
...
Since stylesheets are global by nature, they must be imported in the [Custom `<App>` component](https://nextjs-cn.com/docs/advanced-features/custom-app). This is necessary to avoid class name and ordering conflicts for global styles.
...
CSS Modules for Sass files are an *optional* feature and are only enabled for files with the `.module.scss` extension. Regular [`<link>` stylesheets](https://nextjs-cn.com/docs/api-reference/next/head) and [global Sass styles](https://nextjs-cn.com/blog/next-9-2#built-in-css-support-for-global-stylesheets) are still supported.
