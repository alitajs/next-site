...
> **注意**: Predefined routes take precedence over dynamic routes.
...
> **注意**: Pages that are statically optimized by [automatic static optimization](#automatic-static-optimization) will be hydrated without their route parameters provided (`query` will be empty, i.e. `{}`).
...
> **注意**: If deploying to <PlatformLink /> dynamic routes will work out-of-the-box.
> You do not need to configure custom routes in a `now.json` file.
>
> If you are new to <PlatformLink />, you can learn how to deploy a Next.js app to it in the [_Deploying a Next.js App_ Learn section](https://nextjs-cn.com/learn/basics/deploying-a-nextjs-app).
...
> **注意**: Using router events in `getInitialProps` is discouraged as it may result in unexpected behavior.<br/>
...
> **注意**: `useRouter` is a React hook, meaning it cannot be used with classes.
...
> **注意**: API Routes [do not specify CORS headers](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS), so they'll be **same-origin only** by default.
...
> **注意**: `<Document>`'s `getInitialProps` function is not called during client-side transitions,
> nor when a page is [automatically statically optimized](#automatic-static-optimization).

> **注意**: Make sure to check if `ctx.req` / `ctx.res` are defined in `getInitialProps`.
...
> **注意**: A page that relies on `publicRuntimeConfig` **must** use `getInitialProps` to opt-out of [automatic static optimization](#automatic-static-optimization).
...
> **注意**: Statically generated pages are still reactive: Next.js will hydrate your application client-side to give it full interactivity.
...
> **注意**: If you have a [custom `<App>`](#custom-app) with `getInitialProps` then this optimization will be disabled.

> **注意**: If you have a [custom `<Document>`](#custom-document) with `getInitialProps` be sure you check if `ctx.req` is defined before assuming the page is server-side rendered.
...
To deploy Next.js with <PlatformLink /> see [our guide for Deploying Next.js](https://zeit.co/guides/deploying-nextjs-with-now/) or the [Next.js Learn section about deploying](https://nextjs-cn.com/learn/basics/deploying-a-nextjs-app/deploying-to-zeit-now).
...
> **注意**: Next.js will create a file named `next-env.d.ts` in the root of your project.
...
> **注意**: Next.js does not enable TypeScript's `strict` mode by default.
...
> **注意**: When using `exportTrailingSlash: true` in `next.config.js`, output will be different. For Hybrid AMP pages, output will be `out/page/index.html` and `out/page.amp/index.html`, and for AMP-only pages, output will be `out/page/index.html`
...
> **注意**: If the export path is a filename (e.g. `/readme.md`) and is different than `.html`, you may need to set the `Content-Type` header to `text/html` when serving this content.
...
> **注意**: If your pages don't have `getInitialProps` you may not need `next export` at all, `next build` is already enough thanks to [automatic static optimization](#automatic-static-optimization).
